...
export class DashboardHomeComponent implements AfterViewInit, OnDestroy {
  ...
  kpis = computed(() => {
    ...
    const allUnits = portfolio.flatMap(g => g.units);
    const totalUnits = allUnits.length;
    let occupancyRate = 0;

    if (totalUnits > 0) {
      const allBookings = this.portfolioService.bookings().filter(b => b.status === 'confirmed' || b.source === 'blocked');
      ...
      if (mode === 'month') {
        ...
        // for 'all', defaults to last 365 days or similar? Let's use current year for simplicity or logical bound
        start = new Date(current.getFullYear(), 0, 1);
        end = new Date(); // up to today
      }

      const totalPossibleNights = totalUnits * ((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
      let bookedNights = 0;

      allBookings.forEach(b => {
        // Intersect booking with filter period
        const bStart = b.startDate < start ? start : b.startDate;
        const bEnd = b.endDate > end ? end : b.endDate;

        if (bStart < bEnd) {
          const nights = (bEnd.getTime() - bStart.getTime()) / (1000 * 60 * 60 * 24);
          bookedNights += nights;
        }
      });

      if (totalPossibleNights > 0) {
        occupancyRate = (bookedNights / totalPossibleNights) * 100;
      }
    }
