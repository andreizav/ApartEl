...
export class AuthService {
  ...
  private getJwtSecret(): string {
    ...
    if (!secret) {
      ...
      console.warn(
        'SECURITY WARNING: Using insecure default JWT secret. Do not use this in production!',
      );
  ...
  async login(loginDto: LoginDto) {
    ...
    // Optimization: Use database-level case-insensitive lookup to avoid O(N) memory allocation and processing
    // Expected impact: Faster authentication and significantly less memory overhead on large staff tables.
    ...
    // Map SQLite tinyint (0/1) back to JS boolean
    user.online = Boolean(user.online);
  ...
  async register(registerDto: RegisterDto) {
    ...
    // Optimization: Use database-level case-insensitive lookup to avoid O(N) memory allocation and processing
    // Expected impact: Faster registration and significantly less memory overhead on large staff tables.
    const existingUsers = await this.prisma.$queryRaw<
      any[]
    >`SELECT 1 FROM "Staff" WHERE LOWER(email) = LOWER(${email}) LIMIT 1`;
