...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    ...
    // ⚡ Bolt: Replaced O(N) findMany + array search with O(1) indexed DB lookup
    // SQLite doesn't support mode:'insensitive', so we use $queryRaw
    const staffRaw = await this.prisma.$queryRaw<any[]>`
            SELECT * FROM Staff WHERE LOWER(email) = LOWER(${email}) LIMIT 1
        `;
    const user = staffRaw[0];

    if (!user) {
      throw new UnauthorizedException('User not found');
    }

    // ⚡ Bolt: Convert SQLite boolean integers to actual booleans for the application layer
    user.online = Boolean(user.online);
    user.unreadCount = Number(user.unreadCount);
