  ...
  async scrape(url: string): Promise<{ title: string; image: string }> {
    ...
    try {
      ...
      const data = await page.evaluate(() => {
        ...
        const airbnbTitle = document.querySelector(
          'h1[elementtiming="LCP-target"], h1#title_0, [data-testid="listing-title"]',
        )?.textContent;

        // Booking-specific selectors
        const bookingTitle = document.querySelector(
          'h1#hp_hotel_name, .pp-header__title, [data-testid="property-name"]',
        )?.textContent;
        ...
        // Fallback for image
        if (!image) {
          const airbnbImg = document
            .querySelector(
              'img[data-original-uri], [data-testid="pdp-gallery-image-0"] img',
            )
            ?.getAttribute('src');
          if (airbnbImg) image = airbnbImg;

          if (!image) {
            const bookingImg = document
              .querySelector('.bh-photo-grid-item img, .hotel_main_content img')
              ?.getAttribute('src');
      ...
      this.logger.log(
        `Scraped: Title="${data.title}", Image="${data.image?.substring(0, 50)}..."`,
      );
