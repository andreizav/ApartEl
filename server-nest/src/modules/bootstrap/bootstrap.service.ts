...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // ⚡ Bolt Optimization: Fetch all independent root entities concurrently to minimize database round-trip times
    const [
      groups,
      bookings,
      clients,
    ...
    // ⚡ Bolt Optimization: Resolve N+1 query issue by using `flatMap` to collect unit IDs and querying all mappings and connections in bulk
