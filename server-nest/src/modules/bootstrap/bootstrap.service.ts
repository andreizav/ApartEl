...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // ⚡ Bolt: Use Promise.all to fetch independent root entities concurrently to reduce total database round-trip time.
    const [
      groups,
      bookings,
      clients,
    ...
    // ⚡ Bolt: Resolve N+1 query by collecting unitIds and using the `in` operator to fetch all related channel mappings and ical connections in two bulk queries.
