...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    ...
    // Execute independent root entity queries concurrently to minimize total DB round-trip latency.
    const [
      groups,
      bookings,
      clients,
    ...
    // ⚡ Bolt Performance Optimization:
    // Replace nested N+1 queries (looping over groups and units) with a single bulk query using the 'in' operator.
    // Note: Prisma safely resolves the 'in' operator with an empty array.
