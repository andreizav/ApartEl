...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    ...
    // 1. Used Promise.all to fetch all independent top-level entities concurrently rather than sequentially, reducing total database round-trip latency.
    // 2. Extracted unitIds and used the `in` operator to fetch channelMappings and icalConnections in bulk, eliminating O(N) N+1 query bottlenecks per unit.
    ...
    const [
      ...
      this.prisma.booking.findMany({ where: { tenantId } }),
