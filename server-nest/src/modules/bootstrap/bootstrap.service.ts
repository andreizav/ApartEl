...
export class BootstrapService {
    ...
    async getBootstrapData(tenantId: string, user: any, tenant: any) {
        // ⚡ Bolt: Fetch all root entities concurrently for ~2.5x speedup
        ...
        // ⚡ Bolt: Fix N+1 query problem by batching channel mapping and ical connection queries
        const allUnitIds = groups.flatMap(group => group.units.map(u => u.id));
