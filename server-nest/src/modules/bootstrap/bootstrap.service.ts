...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // Fetch all independent root entities concurrently
    // ⚡ Bolt: Using Promise.all to reduce database round-trips from 7 sequential to 1 concurrent round.
    const [
      groups,
      bookings,
      clients,
    ...
    // ⚡ Bolt: Resolving N+1 queries by collecting all IDs and fetching using `in`.
