...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // Performance optimization: Group independent root entity queries concurrently using Promise.all
    // to minimize total database wait time rather than running them sequentially.
    const [
      groups,
      bookings,
      clients,
    ...
    // Performance optimization: Resolve N+1 queries for unit relations.
    // Rather than querying mapping and ical connections individually in loops, extract unit IDs
    // and fetch them concurrently using single bulk `in` queries.
    const unitIds = groups.flatMap((group) =>
      group.units.map((unit) => unit.id),
    );
    const [channelMappings, icalConnections] =
      unitIds.length > 0
        ? await Promise.all([
