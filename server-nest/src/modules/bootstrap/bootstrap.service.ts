...
export class BootstrapService {
    ...
    async getBootstrapData(tenantId: string, user: any, tenant: any) {
        // ⚡ Bolt Optimization: Fetch root entities concurrently using Promise.all
        // This eliminates sequential database round-trips and significantly reduces
        // the initial load latency of getBootstrapData.
        const [
            groups,
            bookings,
            clients,
        ...
        // ⚡ Bolt Optimization: Resolve N+1 queries by bulk fetching mappings and icals
        // Extract all unit IDs and use the `in` operator to fetch relations in two queries
        // rather than doing it in a nested loop.
