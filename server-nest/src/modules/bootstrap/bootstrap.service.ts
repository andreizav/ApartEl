...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // Fetch all independent data concurrently to minimize database round-trips
    const [
      groups,
      bookings,
      clients,
    ...
    // Collect all unit IDs to eliminate N+1 queries when fetching mappings/connections
    const unitIds = groups.flatMap((g) => g.units.map((u) => u.id));

    // Fetch channel mappings and ical connections concurrently in a single batch query
  ...
  private async clearTenantData(tenantId: string, excludeStaffId?: string) {
    // Fetch all independent root IDs concurrently
    const [groups, clients, categories] = await Promise.all([
    ...
    // Get all units (dependent on groups)
