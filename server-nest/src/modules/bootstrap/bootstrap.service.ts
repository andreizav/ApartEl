...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // Fetch root entities concurrently using Promise.all to minimize database round-trip times
    const [
      groups,
      bookings,
      clients,
    ...
    // Resolve N+1 query problem by batching queries using Prisma's `in` operator
    ...
    if (unitIds.length > 0) {
      const [mappings, icals] = await Promise.all([
