...
export class BootstrapService {
    ...
    async getBootstrapData(tenantId: string, user: any, tenant: any) {
        // ⚡ Bolt: Fetch independent root entities concurrently to reduce total DB roundtrip time
        ...
        // ⚡ Bolt: Eliminate N+1 query loop by fetching child relations in bulk using flatMap + in operator
        const unitIds = groups.flatMap(group => group.units.map(u => u.id));
