...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    ...
    // 1. Replaced sequential awaits with Promise.all to fetch independent root entities concurrently
    // 2. Eliminated N+1 query loop for channelMappings and icalConnections by using nested relational filtering
    // Impact: Reduces initial bootstrap load time significantly by avoiding multiple database round-trips
    const [
      groups,
      bookings,
      clients,
