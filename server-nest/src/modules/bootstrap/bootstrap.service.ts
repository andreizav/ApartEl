...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // Get portfolio with units, mappings, and ical connections in a single query to avoid N+1 queries
    ...
    // Extract channel mappings and ical connections from units
    const channelMappings: any[] = [];
    const icalConnections: any[] = [];

    for (const group of groups) {
      for (const unit of group.units) {
        if (unit.channelMappings) {
          channelMappings.push(...unit.channelMappings);
          // Remove nested mappings from the unit to keep the payload identical to before
          delete (unit as any).channelMappings;
        }
        if (unit.icalConnections) {
          icalConnections.push(...unit.icalConnections);
          // Remove nested connections from the unit to keep the payload identical to before
          delete (unit as any).icalConnections;
        }
      }
    }
  ...
  private async seedTenantData(tenantId: string) {
    ...
    const units = [
      ...
      },
      {
        ...
        photos: JSON.stringify([
          {
            url: 'https://images.unsplash.com/photo-1502005229762-cf1b2da7c5d6?q=80&w=800&auto=format&fit=crop',
            caption: 'View',
          },
        ]),
      },
    ...
    await this.prisma.transactionCategory.create({
      data: {
        ...
        subCategories: {
          create: [
            { id: `tsc-ut-${Date.now()}`, name: 'Utilities' },
            { id: `tsc-int-${Date.now()}`, name: 'Internet' },
          ],
        },
      },
    });
    ...
    await this.prisma.transactionCategory.create({
      data: {
        ...
        subCategories: {
          create: [
            { id: `tsc-rep-${Date.now()}`, name: 'Repairs' },
            { id: `tsc-sup-${Date.now()}`, name: 'Supplies' },
          ],
        },
      },
    });
      ...
      data: [
        {
          id: `m1-${Date.now()}`,
          clientId: client1Id,
          text: 'Hi, checking availability for next weekend.',
          sender: 'client',
        ...
        },
        {
          id: `m2-${Date.now()}`,
          clientId: client1Id,
          text: 'We have Loft 101 available!',
          sender: 'bot',
    ...
    currentEnd.setDate(now.getDate() + 3);

    const futureStart = new Date(now);
    futureStart.setDate(now.getDate() + 10);
    const futureEnd = new Date(now);
    futureEnd.setDate(now.getDate() + 15);
    ...
    await this.prisma.transaction.create({
      data: {
        id: `t-inc-1-${Date.now()}`,
        tenantId,
        date: pastStart.toISOString(),
        property: units[0].name,
        category: 'Accommodation',
        subCategory: 'Booking Revenue',
        description: 'Airbnb Payout #123',
        amount: 500,
        currency: 'USD',
        type: 'income',
        unitId: units[0].id,
    ...
    await this.prisma.transaction.create({
      data: {
        ...
        property: units[1].name,
        category: 'Operations',
        subCategory: 'Internet',
        description: 'Monthly Fiber Bill',
        amount: 65,
        currency: 'USD',
        type: 'expense',
        unitId: units[1].id,
    ...
    await this.prisma.inventoryCategory.create({
      data: { id: bedCat, tenantId, name: 'Bedding' },
    });
    await this.prisma.inventoryCategory.create({
      data: { id: bathCat, tenantId, name: 'Bathroom' },
    });
