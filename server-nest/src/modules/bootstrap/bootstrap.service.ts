...
export class BootstrapService {
    ...
    async getBootstrapData(tenantId: string, user: any, tenant: any) {
        // OPTIMIZATION: Fetch independent root entities concurrently with Promise.all to minimize DB round-trips
        const [
            groups,
            bookings,
            clients,
        ...
        // OPTIMIZATION: Resolve N+1 queries inside loops using flatMap and the `in` operator
