...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // ⚡ Bolt: Fetch all independent root entities concurrently to minimize database round trips
    const [groups, bookings, clients, staff, transactions, inventory] =
      await Promise.all([
    ...
    // ⚡ Bolt: Resolve N+1 queries by collecting all unit IDs and fetching related entities in bulk
    const unitIds = groups.flatMap((group) =>
      group.units.map((unit) => unit.id),
    );

    const [channelMappings, icalConnections, tenantData] = await Promise.all([
      ...
      // Get tenant settings concurrently with the other remaining queries
