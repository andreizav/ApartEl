...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // ⚡ Bolt: Fetch all independent root entities concurrently to reduce total database round-trip time.
    const [
      groups,
      bookings,
      clients,
    ...
    // ⚡ Bolt: Resolve N+1 query issue for channel mappings and ical connections.
    // Batch fetch using the `in` operator instead of looping and querying sequentially.
