...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    ...
    // Solved N+1 queries when fetching channel mappings and iCal connections for units.
    // Instead of querying inside loops, collect IDs using flatMap and perform a single
    // bulk query using the 'in' operator. Use Promise.all for concurrent fetching.
    // Estimated Impact: Reduces database queries from O(N) to O(1) bulk fetch, improving load times.
