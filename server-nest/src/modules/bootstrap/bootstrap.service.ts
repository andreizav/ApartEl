...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // ⚡ Bolt: Optimize bootstrap by fetching all independent root entities concurrently
    // and using batched queries for nested relations to eliminate N+1 loops.
    // Expected impact: Significant reduction in database round-trip times and faster initial app load.
    const [
      groups,
      bookings,
      clients,
    ...
    // Get channel mappings and ical connections from units in batch
