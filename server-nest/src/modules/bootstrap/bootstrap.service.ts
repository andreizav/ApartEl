...
export class BootstrapService {
  ...
  async getBootstrapData(tenantId: string, user: any, tenant: any) {
    // ⚡ Bolt: Fetch independent root entities concurrently using Promise.all
    // This eliminates sequential round-trip delays, reducing the total query time.
    const [
      groups,
      bookings,
      clients,
    ...
    // ⚡ Bolt: Resolve N+1 queries for nested relationships
    // By collecting unitIds and using the 'in' operator, we perform two queries
    // instead of 2 * (number of units) queries.
