...
export class TransactionsService {
  ...
  async syncUnitIncome(tenantId: string, unitId: string) {
    ...
    // ⚡ Bolt Performance Optimization: Replace N+1 sequential inserts with batch createMany
    // Avoids massive disk synchronization overhead from individual SQLite INSERT statements
    if (newTransactions.length > 0) {
      await this.prisma.transaction.createMany({ data: newTransactions });
      createdTransactions.push(...newTransactions);
    }
