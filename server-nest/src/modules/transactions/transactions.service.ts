...
export class TransactionsService {
  ...
  async syncUnitIncome(tenantId: string, unitId: string) {
    ...
    // 3. Prepare data for missing bookings
    let i = 0;
    for (const booking of bookings) {
      if (!existingBookingIds.has(booking.id)) {
        ...
        newTransactionsData.push({
          id: txData.id || `tx-${baseTime + i++}`,
          tenantId,
          date: new Date(txData.date),
          property: txData.property ?? '',
    ...
    if (newTransactionsData.length > 0) {
      // ⚡ Bolt: Batch insert transactions to eliminate N+1 database queries
      // This prevents O(N) database round-trips for units with many missing bookings
      await this.prisma.transaction.createMany({
        data: newTransactionsData,
      });

      // Retrieve the newly created transactions to match the original return type
      const newBookingIds = newTransactionsData.map((tx) => tx.bookingId);
      createdTransactions = await this.prisma.transaction.findMany({
        where: {
          tenantId,
          unitId,
