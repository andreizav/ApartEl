...
export class TransactionsService {
  ...
  async createMany(tenantId: string, transactions: any[]) {
    if (!transactions.length) return [];

    // ⚡ Bolt: Batch create to eliminate N+1 database roundtrips during sync
    const creates = transactions.map((tx, i) =>
      this.prisma.transaction.create({
        data: {
          // Use Date.now() + i to prevent ID collisions in a loop
  ...
  async syncUnitIncome(tenantId: string, unitId: string) {
    ...
    if (missingTransactions.length > 0) {
      const batchCreates = await this.createMany(tenantId, missingTransactions);
      createdTransactions.push(...batchCreates);
    }
