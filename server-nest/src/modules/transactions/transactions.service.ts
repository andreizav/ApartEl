...
export class TransactionsService {
  ...
  async syncUnitIncome(tenantId: string, unitId: string) {
    ...
    // ⚡ Bolt: Batching inserts to avoid N+1 queries during sync
    ...
    for (const booking of bookings) {
      if (!existingBookingIds.has(booking.id)) {
        ...
        createPromises.push(this.prisma.transaction.create({ data: txData }));
      }
    }

    let createdTransactions: any[] = [];
    if (createPromises.length > 0) {
      createdTransactions = await this.prisma.$transaction(createPromises);
    }
