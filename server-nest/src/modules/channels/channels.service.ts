...
export class ChannelsService {
  ...
  async updateMappings(tenantId: string, list: any[]) {
    ...
    // ⚡ Bolt: Resolve N+1 query bottleneck by using a single transaction to execute batch upserts,
    // reducing SQLite disk synchronization overhead and database round-trips.
  ...
  async updateIcal(tenantId: string, list: any[]) {
    ...
    // ⚡ Bolt: Resolve N+1 query bottleneck by executing iCal upserts concurrently in a transaction,
    // drastically reducing total operation latency.
  ...
  async sync(tenantId: string) {
    ...
    // ⚡ Bolt: Fetch independent root entities concurrently to minimize database round-trip times.
    const [currentMappings, currentIcals] = await Promise.all([
      this.getMappings(tenantId),
      this.getIcal(tenantId),
    ]);
    const updatedMappings: any[] = [];
    const updatedIcals: any[] = [];

    // ⚡ Bolt: Batch create and update operations into a single transaction to avoid N+1 queries inside loop.
    ...
    for (const { unit, groupName } of allUnits) {
      ...
      if (existingMap) {
        syncPromises.push(
          this.prisma.channelMapping.update({
        ...
        const newId = `cm-${unit.id}`;
        syncPromises.push(
          this.prisma.channelMapping.create({
            data: {
              id: newId,
              unitId: unit.id,
              unitName: unit.name,
      ...
      if (existingIcal) {
        syncPromises.push(
          this.prisma.icalConnection.update({
        ...
        const newId = `ical-${unit.id}`;
        syncPromises.push(
          this.prisma.icalConnection.create({
            data: {
              id: newId,
              unitId: unit.id,
              unitName: unit.name,
    ...
    console.log(
      `[Channels] Starting manual iCal sync for tenant ${tenantId}...`,
    );
    for (const conn of updatedIcals) {
      if (conn.importUrl) {
        console.log(
          `[Channels] Syncing iCal for tenant ${tenantId}, unit ${conn.unitId} from ${conn.importUrl}`,
        );
    ...
    // Execute all collected updates/creates concurrently.
    await this.prisma.$transaction(syncPromises);
