...
export class ChannelsService {
  ...
  async updateMappings(tenantId: string, list: any[]) {
    ...
    // ⚡ Bolt: Batch database operations using $transaction to resolve N+1 performance bottleneck
    // Replaces O(N) sequential database calls with a single round-trip, significantly reducing latency.
    const promises = list.map((mapping) =>
  ...
  async sync(tenantId: string) {
    ...
    // ⚡ Bolt: Batch database operations using $transaction to resolve N+1 performance bottleneck
    // Consolidates map and ical updates/creations into a single db transaction
    ...
    if (manualSyncPromises.length > 0) {
      await this.prisma.$transaction(manualSyncPromises);
    }
