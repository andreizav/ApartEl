...
export class ChannelsService {
  ...
  async updateMappings(tenantId: string, list: any[]) {
    ...
    // ⚡ Bolt: Batch upserts in a single transaction to eliminate N+1 queries
    // Impact: Significant reduction in DB round-trips and SQLite disk sync overhead
  ...
  async sync(tenantId: string) {
    ...
    // ⚡ Bolt: Collect DB operations to execute in a single batch
    // Impact: Significant reduction in DB round-trips for heterogeneous operations
